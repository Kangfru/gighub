...
export function renderBandCard(band: BandResponse): string {
  const roleClasses = band.role === 'LEADER'
    ? 'badge-leader text-purple-700'
    : 'badge-gradient text-white'
  const roleText = band.role === 'LEADER' ? '👑 리더' : '🎵 멤버'

  return `
    <div
      class="bg-white rounded-2xl shadow-lg p-8 card-hover cursor-pointer fade-in overflow-hidden relative"
    ...
    >
      ...
      <div class="relative">
        <div class="flex justify-between items-start mb-4">
          <h3 class="text-2xl font-bold text-gray-900">${band.name}</h3>
        ...
        ${
          band.description
            ? `<p class="text-gray-600 text-sm mb-4 line-clamp-2">${band.description}</p>`
