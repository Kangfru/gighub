...
export async function renderBandsPage(): Promise<void> {
  ...
  try {
    ...
    app.innerHTML = `
      ...
      <div class="min-h-screen py-8 bg-gray-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="flex justify-between items-center mb-10 fade-in">
            <div>
              <h1 class="text-4xl font-bold text-gray-900 mb-2">내 밴드</h1>
              <p class="text-gray-600">함께 음악을 만들어가는 공간</p>
            </div>
            <button
              onclick="window.showCreateBandModal()"
              class="btn-gradient text-white font-semibold px-8 py-4 rounded-xl shadow-lg flex items-center gap-2"
            >
          ...
          ${
            ...
            <div class="text-center mt-20 fade-in">
              ...
              <p class="text-2xl font-bold text-gray-700 mb-3">아직 밴드가 없습니다</p>
              <p class="text-gray-500 mb-8">밴드를 만들거나 초대 코드로 가입해보세요!</p>
              <button
                onclick="window.showCreateBandModal()"
                class="btn-gradient text-white font-semibold px-10 py-4 rounded-xl shadow-lg inline-flex items-center gap-2"
              >
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                  <path fill-rule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" clip-rule="evenodd"></path>
                </svg>
      ...
      <!-- 밴드 생성 모달 -->
      <div id="create-band-modal" class="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm hidden items-center justify-center z-50 px-4">
        <div class="bg-white rounded-3xl p-10 w-full max-w-md scale-in shadow-2xl">
          <h2 class="text-3xl font-bold text-gray-900 mb-8">새 밴드 만들기</h2>
          <form id="create-band-form" class="space-y-4">
            ...
            <div>
              ...
              <textarea
                id="band-description"
                rows="3"
                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none transition-all resize-none"
                placeholder="밴드에 대해 간단히 소개해주세요"
              ></textarea>
            </div>

            <div id="modal-error" class="bg-red-50 text-red-600 text-sm px-4 py-3 rounded-xl hidden"></div>
            ...
            <div class="flex gap-3 pt-2">
              ...
              <button
                type="submit"
                class="flex-1 btn-gradient text-white font-semibold py-4 px-6 rounded-xl shadow-lg"
              >
                만들기
              </button>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="text-center mt-20 fade-in">
            ...
            <button
              ...
              class="btn-gradient text-white font-semibold px-10 py-4 rounded-xl shadow-lg"
            >
              다시 시도
            </button>
          </div>
